...
let passwordConfirmField = document.querySelector("#passwordConfirmInput");
...
dobInput.addEventListener("input", () => {
  ...
  let age = getYearsSince(value);
  if (age < 13) {
    dobInput.setCustomValidity("You need to be at least 13 years old.");
    document.querySelector("#dobFeedback").textContent =
      "You need to be at least 13 years old.";
  } else {
...
document.querySelectorAll("input").forEach((input) => {
  input.addEventListener("input", () => {
    if (form.classList.contains("was-validated")) {
      submitBtn.disabled = !input.validity.valid;
    }
  });
});
